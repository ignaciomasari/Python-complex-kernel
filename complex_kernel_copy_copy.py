...
MRF_SOLVER = 'GC'# 'GC', 'ICM', 'LBP'
KERNEL_TYPE = 'complex' #'real', 'complex', 'complex_sym', 'complex_positive', 'complex_sym_2', 'complex_sym_3'
...
def complex_symmetrical_3_kernel(gamma):
    def gaussian(X, Y):
        X_comp = np.array(X[:,:,0], dtype='complex128')
        X_comp.imag = X[:,:,1]
        Y_comp = np.array(Y[:,:,0], dtype='complex128')
...
def SVM_MRF_LBP(decision_image, lambda_, num_iterations):
    # Compute data cost
    data_cost = np.concatenate([(-decision_image)[:, :, np.newaxis], decision_image[:, :, np.newaxis]], axis=2).astype(np.float32)
    data_cost *= lambda_[0]
    ...
    for _ in range(num_iterations):
            # Perform one iteration of graph cut
            g.maxflow()

            # Get the current labeling
            labels = g.get_grid_segments(nodeids)

            # Update the data cost based on the current labeling
            data_cost[:, :, 0, 0] = -labels.astype(float)
            data_cost[:, :, 1, 0] = labels.astype(float)

            # Update the graph with the updated data cost
            g.add_grid_tedges(nodeids, data_cost[:, :, 0, 0], data_cost[:, :, 1, 0])
    ...
    # Return labeling and energy (optional)
    energy = g.maxflow()
    return energy, labels
...
if __name__=="__main__":
    ...
    if (USE_MRF):
        ...
        match KERNEL_TYPE:
            ...
            case 'complex_sym':
                pass
            case 'complex_sym_3':
                kernel_function = complex_symmetrical_3_kernel
    ...
    if USE_POWELL:
        ...
        if DATASET=='Flevoland':
            C = 15.687306032778462
            gamma = 3.5479085314651417
        else:
            C = 0.126850242225449
            gamma = 1.7418257862290536
    ...
    if (USE_MRF): # MRF
        ...
        for pair in combinations(classes_labels,2):
            ...
            for iter in range(MRF_ITERATIONS):
                ...
                if MRF_SOLVER == 'GC':
                    ...
                    energy, classification = SVM_MRF_LBP(decision_image, lambda_, num_iterations=10)
                    if energy < energy_min:
                        energy_min = energy
                        classification_optimal = classification
            ...
            elif MRF_SOLVER == 'GC' or MRF_SOLVER == 'LBP':
        ...
        np.save("./Output/San Francisco/complex_mrf10_complete_lambda1_powell_pddp2000.npy", max_vote)
        ...
        y_predicted_flatten = np.zeros((train_map_flatten.shape), dtype=np.uint8) 
        decision_flatten = np.zeros((train_map_flatten.shape)) 
        for division in range(Predict_batches):
            y_predicted_flatten[
                y_predicted_flatten.size // Predict_batches * division : y_predicted_flatten.size // Predict_batches * (division + 1)
                ] = clf.predict(complex_image[
                y_predicted_flatten.size // Predict_batches * division : y_predicted_flatten.size // Predict_batches * (division + 1)
                ])                 
        y_predicted_flatten[y_predicted_flatten.size // Predict_batches * Predict_batches:] = clf.predict(complex_image[y_predicted_flatten.size // Predict_batches * Predict_batches:])
        y_predicted_image = np.reshape(y_predicted_flatten,(train_map.shape))
        np.save("./Output/Flevoland/complex_sym_3_powell_nomrf.npy", y_predicted_image)
